...
/**
 * A Collection / Box Set - a set of [Album]s that share the same `grouping` metadata
 * value, e.g. a 222-disc box set where each disc is its
...
data class MusicCollection(
     ...
     * per-album `coverArt`), so this is the documented fallback: the first disc's own artwork.
     */
    val artworkAlbum: Album? get() = albums.firstOrNull()

    /**
     * Up to 3 albums for the stacked-cover presentation: index 0 is the dominant/front cover
     * (same album as [artworkAlbum]), 1 is the
